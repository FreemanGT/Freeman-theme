...
## [freeman-core 1.11.51] — 2026-05-13

- VariationSwatches — the variation pickers on the single product page (including inside quick-view modals), on the shop / category / search archives, and in product-grid widgets now order each attribute's options by size instead of by raw database insertion order. Numbers and fractions sort ascending (`28` … `33`, plus comma decimals and mixed/simple fractions like `36 2/3` / `37 1/3` — French/EU shoe sizing); clothing size tokens sort by size, not alphabetically (`XS` < `S` < `M` < `L` < `XL` < `XXL` < `3XL` …, also `Small`/`Medium`/`Large`/`X-Large`/`XX-Large` spellings and `One Size`); and a value that merely starts with a number — `9-10 חודשים`, `12-18 months`, `Size 36` — sorts by that number. Only when nothing in an attribute is recognisable as a size does it fall back to the order configured in WooCommerce (taxonomy term order / typed custom-attribute order). Out-of-stock values are demoted to the end of the row. Root cause: `WC_Product_Variable::get_variation_attributes()` returns values in raw DB row order (its data store SELECTs `meta_value` with no `ORDER BY`). New non-legacy `Attribute_Order` helper does the sort; `Etucart_VS_Frontend::render_variable()` and `Etucart_VS_Archive::prepare_product_data()` each call it after their existing optional out-of-stock-hide filter — one line each in `legacy/includes/class-frontend.php` and `class-archive.php`, Hard Rule #3 single-line exceptions of the same shape as 1.11.40's `do_action` addition. No new hooks / options; `tests/baseline-hooks.txt` shifts a few line numbers only. (Supersedes the in-progress 1.11.50 work — never released; consolidated here.)
