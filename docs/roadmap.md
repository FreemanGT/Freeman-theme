...
| 4.5 | VariationSwatches WPC Bundles + FBT compatibility (full form-field forwarding + bundle-marker capture skip) | freeman-core 1.11.17 | 2026-04-30 | _this PR_ |
...
**4.5 — VariationSwatches WPC Bundles + FBT compatibility (P1 bugfix)** ✅ shipped 1.11.17 (_this PR_, 2026-04-30)
...
- Markers verified against actual plugin source on 2026-04-30 (wp.org slugs `woo-bought-together` for FBT, `woo-product-bundle` for Bundles). 1.11.13's first attempt used `wcfbt_` based on a guess and broke FBT sites — corrected in 1.11.14. 1.11.17 added the missing template hook because 1.11.14 still couldn't find the marker (the hidden input was never injected).
