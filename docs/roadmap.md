...
- **6.4 — Admin facet config** (targets 1.12.21) — per-attribute matrix (is-facet / type / order / hide-on-category) via `freeman_core/module_page/shop_filters`. Flag `freeman_core_shop_filters_admin_config_enabled`.
- **6.5a — SEO policy** (targets 1.12.22) — filtered URLs get canonical → clean URL + `noindex,follow` (query-string params only; no rewrite rules). Flag `freeman_core_shop_filters_seo_policy_enabled`.
- **6.5c — On-sale / in-stock facets** (deferred from 6.5b; targets 1.12.23) — the remaining `wc_product_meta_lookup`-sourced numeric facets. Reuses `frontend_enabled`. Promote on need.

> **OPEN — AWS search grid (in progress):** QA narrowed it: on search the grid *does* respond to our `tax_query` (so it IS the main query) but sold-out-size products remain → AWS overrides our `post__in` / `the_posts` by running *after* us. **1.12.20** moves our `the_posts` enforcement to priority `99999` to run last. **If this still doesn't work**, AWS renders the grid from a *separate* query (e.g. an Elementor Loop/Archive widget query, not the main query) — then the fix is to hook AWS's own results filter or the widget query, not the main query. Awaiting QA.
