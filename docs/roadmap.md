...
- **6.4 — Admin facet config** (targets 1.12.20) — per-attribute matrix (is-facet / type / order / hide-on-category) via `freeman_core/module_page/shop_filters`. Flag `freeman_core_shop_filters_admin_config_enabled`.
- **6.5a — SEO policy** (targets 1.12.21) — filtered URLs get canonical → clean URL + `noindex,follow` (query-string params only; no rewrite rules). Flag `freeman_core_shop_filters_seo_policy_enabled`.
- **6.5c — On-sale / in-stock facets** (deferred from 6.5b; targets 1.12.22) — the remaining `wc_product_meta_lookup`-sourced numeric facets. Reuses `frontend_enabled`. Promote on need.

> **OPEN — AWS search grid:** the 1.12.18 `the_posts` safety net did **not** fix the sold-out-size product appearing on Advanced Woo Search results — so AWS is not rendering the grid from the main query loop (likely its own template/loop or a replaced query). Next step: integrate with AWS directly (its result-IDs filter / `aws_*` hooks) or constrain its query. To resume.
