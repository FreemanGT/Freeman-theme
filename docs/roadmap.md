...
- **6.4 — Admin facet config** (targets 1.12.14) — per-attribute matrix (is-facet / type / order / hide-on-category) via `freeman_core/module_page/shop_filters`. Flag `freeman_core_shop_filters_admin_config_enabled`.
- **6.5a — SEO policy** (targets 1.12.15) — filtered URLs get canonical → clean URL + `noindex,follow` (query-string params only; no rewrite rules). Flag `freeman_core_shop_filters_seo_policy_enabled`.
- **6.5c — On-sale / in-stock facets** (deferred from 6.5b; targets 1.12.16) — the remaining `wc_product_meta_lookup`-sourced numeric facets. Reuses `frontend_enabled`. Promote on need.
