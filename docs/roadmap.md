...
**Last updated**: 2026-05-20 (Wave 6 — Shop Filters new module — Phase 6.1 foundation ✅ shipped 1.12.0, Phase 6.2 facet engine ✅ shipped 1.12.1. Prior: Wave 5.1 — Feature Flags admin page — ✅ shipped 1.11.44; Wave 2.2 / 4g — VariationSwatches settings surface graduated, `settings_hub` flag retired — ✅ shipped 1.11.45)
**Owner**: Yiftach
...
- **6.3a — Frontend read path** (targets 1.12.2) — `Query_Builder` (index → facet-engine glue, moved from 6.2) + `[freeman_shop_filters]` shortcode + admin-AJAX query endpoint + JS grid-swap (reuses InfiniteScroll container detection; fetches the filtered front-end URL for Elementor markup parity) + checkbox facets. Flag `freeman_core_shop_filters_frontend_enabled`.
