...
- **6.4 — Admin facet config** (targets 1.12.13) — per-attribute matrix (is-facet / type / order / hide-on-category) via `freeman_core/module_page/shop_filters`. Flag `freeman_core_shop_filters_admin_config_enabled`.
- **6.5a — SEO policy** (targets 1.12.14) — filtered URLs get canonical → clean URL + `noindex,follow` (query-string params only; no rewrite rules). Flag `freeman_core_shop_filters_seo_policy_enabled`.
- **6.5b — Numeric facets + sort** (targets 1.12.15) — price (rendered on top) / on-sale / in-stock facets sourced from `wc_product_meta_lookup`, **plus the sort dropdown + default-sort setting** (the deferred 6.3a orderby keys); `Url_State` already parses `min_price`/`max_price` + whitelists `orderby`. By decision, price + sort + default-sort ship **together** in this one phase. Reuses `frontend_enabled`.
