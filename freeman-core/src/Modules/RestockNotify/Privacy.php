 ...
 * hard-delete rows. It scrubs PII columns (`customer_name` → empty string,
 * `customer_email` → non-PII row tombstone) and sets `status='unsubscribed'`.
 * The row stays as an audit trail and the stock monitor can no longer match
 * the email on future restocks.
 *
